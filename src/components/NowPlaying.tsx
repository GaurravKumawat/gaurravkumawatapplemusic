import { ChevronDown, Pause, Play, Repeat, Repeat1, Shuffle, SkipBack, SkipForward, MoreHorizontal, Heart, FileText, Play as PlayIcon } from "lucide-react";
import { AnimatePresence, motion } from "motion/react";
import { formatTime, formatViews, usePlayer } from "@/lib/player-context";
    ...
    <AnimatePresence>
      {showFull && current && (
        <motion.div
          ...
          exit={{ y: "100%", scale: 0.94, borderRadius: 28, opacity: 0.4 }}
          ...
          <div className="relative h-full flex flex-col pt-safe pb-safe px-6">
            ...
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="min-w-0 flex-1">
                ...
                {views && (
                  <div className="flex items-center gap-1 text-[13px] text-white/50 mt-0.5">
                    <PlayIcon className="h-3 w-3 fill-current" />
                    <span className="tabular-nums">{views} plays</span>
                  </div>
                )}
              </div>
              <button
